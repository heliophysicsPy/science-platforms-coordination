...
RUN mkdir -p /opt/heliocloud-base-w-survey-core

# Copy Welcome.ipynb and requirements.txt to the opt directory
COPY Welcome.ipynb /opt/heliocloud-base-w-survey-core/
COPY requirements.txt /opt/heliocloud-base-w-survey-core/

# Extract notebooks archive into notebooks directory in opt
COPY notebooks.tar.gz /tmp/
RUN mkdir -p /opt/heliocloud-base-w-survey-core/notebooks && \
    tar -xzf /tmp/notebooks.tar.gz -C /opt/heliocloud-base-w-survey-core/notebooks && \
    rm -f /tmp/notebooks.tar.gz

# create PyHC package data dirs in opt directory
RUN mkdir -p /opt/heliocloud-base-w-survey-core/.sunpy /opt/heliocloud-base-w-survey-core/.spacepy/data

# Copy start script to the branch-specific directory and make it executable
COPY start /opt/heliocloud-base-w-survey-core/start
RUN chmod +x /opt/heliocloud-base-w-survey-core/start

# Ensure user (default: jovyan) owns everything in opt with full permissions
RUN chown -R $NB_USER /opt/heliocloud-base-w-survey-core && \
    chmod -R 777 /opt/heliocloud-base-w-survey-core
...
ENTRYPOINT ["/opt/heliocloud-base-w-survey-core/start"]
